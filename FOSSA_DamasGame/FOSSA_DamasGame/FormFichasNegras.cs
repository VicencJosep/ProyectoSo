...
{
    public partial class FormFichasNegras : Form
    {
        ...
        private void VictoriaPorAbandono()
        {
            ...
            MessageBox.Show("Enorabuena, has ganado la partida, el contrincate se ha rendido!");
        ...
        public void GetEstadoPartida()
        {
            this.partida = false;
        }

        public FormFichasNegras(int nForm, Socket server, string Contrincante)
        {
            InitializeComponent();
            this.nForm = nForm;
            this.server = server;
        ...
        private void FormFichasNegras_Load(object sender, EventArgs e)
        {
            ...
            Bitmap BckgndImage = new Bitmap("TableroNegras.png");
            Tablero.Size = new System.Drawing.Size(824, 562);
        ...
        private void Tablero_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            if (turno == true)
            {
                ...
                if ((res != -1) && (aircraft_vector[i_global].Location.Y > e.Y) && (Math.Abs(aircraft_vector[i_global].Location.X - e.X) > 2 * 61 + 5) && (Math.Abs(aircraft_vector[i_global].Location.X - e.X) < 3 * 61) && (Math.Abs(aircraft_vector[i_global].Location.Y - e.Y) > 56) && (Math.Abs(aircraft_vector[i_global].Location.Y - e.Y) < 2 * 56))
                {
        ...
        private void EnviarChat_Button_Click_1(object sender, EventArgs e)
        {
            chat.Items.Add(username + ":- " + textBox1.Text);
            string mensaje = "8/" + nForm.ToString() + "/" + Contrincante + "/" + textBox1.Text + "/";
            textBox1.Text = " ";
        ...
        private void FormFichasNegras_FormClosing(object sender, FormClosingEventArgs e)
        {

            if (partida)
            {
                MessageBox.Show("Al cerrar el form contará como un abandono de partida.");
                string mensaje = "12/" + nForm + "/" + Contrincante + "/";
                byte[] msg = System.Text.Encoding.ASCII.GetBytes(mensaje);
