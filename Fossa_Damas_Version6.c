...
int EliminarUsuario(char nomUsuario[100], MYSQL*conn, MYSQL_RES*resultado, MYSQL_ROW row)
{
	...
	else
	{
		...
		resultado = mysql_store_result(conn);
		row = mysql_fetch_row(resultado);
		sprintf(consulta, "UPDATE Relaciones SET ID_Jugador=-1 WHERE ID_Jugador=%d;", IDUsuario);
		err = mysql_query(conn, consulta);
		if(err != 0)
		{
...
int InvitarJugador(char invitado[100], ListaConectados milista )
{
	int j = 0;
	int encontrado = 0;
	int s;
...
int ComprobarSiYaEstaRegistrado(char usuario[100], MYSQL*conn, MYSQL_RES *resultado, MYSQL_ROW row)
{
	int err;
	char consulta[100];
...
//Que me diga tambien con quien he jugado y las kills de la partida
		...
		else if(codigo == 5) // QUERY 3 
		{
			...
			if(err > 0)
			{
				sprintf(respuesta, "5/%d/%d",numForm, err);
			}
			else
			   sprintf(respuesta, "5/%d/-2",numForm);
		}
...
// No borrar las partidas sinÃ³ cambiar el nombre
		...
		else if(codigo == 11) // FIN DE PARTIDA
		{
			...
			if(err < 0)
			{
				sprintf(respuesta, "11/%d/Error", numForm);
				sprintf(notif, "-7/%d/Error", numForm);
			}
			else
			{
				sprintf(respuesta, "11/%d/Ok", numForm);
				sprintf(notif, "-7/%d/Ok", numForm);
			}
			write(socketPerdedor, notif, strlen(notif));
		}
		else
		{
